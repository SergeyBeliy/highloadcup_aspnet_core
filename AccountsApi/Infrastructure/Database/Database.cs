...
namespace AccountsApi.Database.Infrastructure
{
    public class Database : IDatabase
    {
        ...
        {
            var existing = await AccountContext.Accounts.FirstOrDefaultAsync(s => s.Id == account.Id);
            if (existing == null)
            {
                return false;
            }
            ...
            {
                existing.Interests = account.Interests;
            }
            ...
            {
                existing.SName = account.SName;
            }
            if (account.Premium != null)
            {
                existing.Premium = account.Premium;
            }
            if (account.Sex != null)
            {
            ...
            {
                existing.FName = account.FName;
            }

            AccountContext.Accounts.Update(existing);
            await AccountContext.SaveChangesAsync();
            return true;
        }

        public async Task UpdateLikes(LikeUpdateModel[] likes)
        {

            foreach (var like in likes)
            {
                var existing = await AccountContext.Accounts.FirstOrDefaultAsync(s => s.Id == like.Liker);
                if (existing != null)
                {
                    var likeIds = existing.LikeIds == null ? new List<long>() : existing.LikeIds.ToList();
                    likeIds.Add(like.Likee);
                    existing.LikeIds = likeIds.ToArray();
                    var likeTSs = existing.LikeTSs == null ? new List<long>() : existing.LikeTSs.ToList();
                    likeTSs.Add(like.Ts);
                    existing.LikeTSs = likeTSs.ToArray();
                    AccountContext.Accounts.Update(existing);
                    await AccountContext.SaveChangesAsync();
                }
            }
